...
def addMovieMap(catalog, book):
    #Req 1 año de estreno
    anio_estreno=book["release_year"]
    addMovieMap2(catalog,anio_estreno,book,'release_year')

    #Req 2 Anio Added
    date=book["date_added"]
    addMovieMap2(catalog,date,book,'date_added')
...
def getReq1 (catalog, anio):
    start_time=getTime()
    movies = mp.get(catalog['release_year'], anio)
    ...
    return movies_pais_Peli, round(times,3)

def getReq2 (catalog, date):
    start_time=getTime()
    series = mp.get(catalog['date_added'],date)
    series = me.getValue(series)['books']
    lista_series= lt.newList('ARRAY_LIST')
    for elemento in lt.iterator(series):
        if elemento['type']=='TV Show':
            lt.addLast(lista_series, elemento)
    mer.sort(lista_series, cmpMoviesByTitle)
    end_time=getTime()
    times=deltaTime(start_time,end_time)
    return lista_series, round(times,3)
...
def getReq3(catalog, actor):
    ...
    end_time=getTime()
    times=deltaTime(start_time,end_time)
    return movies,movies_pais_TV, movies_pais_Peli,round(times,3)

def getReq4(catalog, genero):
    start_time = getTime()
    cont_movies = 0
    cont_shows = 0
    contenido = mp.get(catalog['listed_in'], genero)
    contenido = me.getValue(contenido)['books']
    for i in lt.iterator(contenido):
        if i['type'] == 'Movie':
            cont_movies += 1
        else:
            cont_shows += 1
    mer.sort(contenido,cmpMoviesByReleaseYear)
    end_time=getTime()
    times=deltaTime(start_time,end_time)
    return contenido, cont_movies, cont_shows, round(times,3)
...
def getReq7(catalog, top):
    ...
    for i in lt.iterator(generos): 
        peliculas= mp.get(catalog['listed_in2'], i)
        count= me.getValue(peliculas)['count']
        book= me.getValue(peliculas)['books']
        if i is not "":
            lt.addLast(lista_cuenta,[i,count,book])
    ...
    for i in lt.iterator(top_n):
        peliculas= mp.get(catalog['listed_in2'], i[0])
        book= me.getValue(peliculas)['books']      
        for j in lt.iterator(book):
            if i[0] in info_genero.keys():
                stser=j["stream_service"]
                typee=j["type"]
                info_genero[i[0]][stser]+=1
                info_genero[i[0]][typee]+=1
            else: 
...
def getReq8(catalog,top, genero):
    start_time=getTime()
    movies2= mp.get(catalog['listed_in'], genero)
